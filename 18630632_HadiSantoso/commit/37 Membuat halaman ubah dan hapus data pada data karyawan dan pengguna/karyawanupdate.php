...
if(isset($_GET['id'])) {
  ...
  $id = $_GET['id'];
  $findSql = "SELECT P.id, K.nik, K.nama_lengkap, K.handphone, K.email, K.tanggal_masuk, P.username, P.password, P.peran FROM pengguna P INNER JOIN karyawan K ON P.id = K.pengguna_id WHERE K.id = ?";
            ...
            if ($stmt->execute()) {
              $updateKaryawanSql = "UPDATE karyawan SET
                ...
                if ($stmtKaryawan->execute()) {
                  $_SESSION['hasil'] = true;
                  $_SESSION['pesan'] = "Berhasil Ubah Data Karyawan";
                } else {
                  $_SESSION['hasil'] = false;
                  $_SESSION['pesan'] = "Gagal Ubah Data Karyawan";
                }
