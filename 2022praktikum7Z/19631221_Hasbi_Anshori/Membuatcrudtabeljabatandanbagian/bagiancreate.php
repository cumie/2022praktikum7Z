...
?> 
    <div class="alert alert-danger alert-dismissible">
      ...
      Nama Bagian Tidak Boleh Sama
    </div>
<?php
  } else {
    $insertSQL = "INSERT INTO bagian SET 
      nama_bagian = ?,
      karyawan_id = ?,
      lokasi_id = ?";
    ...
    if ($stmt->execute()) {
      $_SESSION['hasil'] = true;
      $_SESSION['pesan'] = "Berhasil Simpan Data Bagian";
    } else {
      $_SESSION['hasil'] = false;
      $_SESSION['pesan'] = "Gagal Simpan Data Bagian";
    }
      ...
      <form action="" method="post">
        ...
        <div class="form-group">
          ...
          <select class="form-control" name="karyawan_id">
            ...
            ?>
            <?php 
              $selectSQL = mysqli_query($conn, "SELECT * FROM karyawan") or die (mysqli_error($conn));
                while($row_karyawan = mysqli_fetch_array($selectSQL)) {
                echo '
                <option value="'.$row_karyawan['id'].'">
        ...
        <div class="form-group">
          ...
          <select class="form-control" name="lokasi_id">
            ...
            <?php 
              $selectSQL = mysqli_query($conn, "SELECT * FROM lokasi") or die (mysqli_error($conn));
                while($row_lokasi = mysqli_fetch_array($selectSQL)) {
                echo '
                <option value="'.$row_lokasi['id'].'">
                  '.$row_lokasi['nama_lokasi'].'
                </option>
                ';
                }
            ?>
